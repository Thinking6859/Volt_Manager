...
class DraftView(ui.View):
    ...
    async def pick_callback(self, i):
        if i.user.id != self.leaders[self.order[self.idx]].id: return await i.response.send_message("차례 아님", ephemeral=True)
        ...
        else: self.create_buttons(); await i.response.edit_message(content=f"🔵 다음 선택: {self.leaders[self.order[self.idx]].mention}", view=self)
    ...
    async def finish(self, i):
        ...
        predict = f"💡 **볼티 분석:** {'🟦 1팀' if s1 >= s2 else '🟥 2팀'} 우세 ({prob if s1 >= s2 else 100-prob}%)"
        ...
        await i.response.edit_message(content="✅ 라인업이 공지되었습니다.", view=None)
...
class NextStepView(ui.View):
    ...
    async def r(self, i, b):
        ...
        db = {r['discord_id']: r for r in res.data}; v = ui.View(); sel = ui.Select(cls=ui.UserSelect, placeholder="새 주장 2명 선택", min_values=2, max_values=2)
        async def cb(i2):
    ...
    @ui.button(label="🔥 리매치 (팀 고정)", style=discord.ButtonStyle.success)
    async def rm(self, i, b):
        for pid in current_match["ids"]: supabase.rpc('increment_participation', {'user_id': pid}).execute()
        await bot.get_channel(PUBLIC_CHANNEL_ID).send("🔥 **팀 변경 없이 한 번 더!**"); await i.response.edit_message(content="기록 완료", view=None)
    @ui.button(label="🏁 종료", style=discord.ButtonStyle.secondary)
    async def e(self, i, b): await i.response.edit_message(content="✅ 종료", view=None)
...
class AdminManageView(ui.View):
    def __init__(self, all_users):
        ...
        async def cb(i):
            ...
            await i.followup.send("✅ 권한 동기화 완료!", ephemeral=True)
...
class NoticeDeployView(ui.View):
    ...
    async def b_reg(self, i, b):
        v = ui.View(timeout=None); btn = ui.Button(label="클린원 등록/수정", style=discord.ButtonStyle.primary)
        async def cb(i2):
    ...
    async def b_rank(self, i, b):
        v = ui.View(timeout=None); btn = ui.Button(label="📊 내 승점/랭킹 확인하기", style=discord.ButtonStyle.success)
        async def cb(i2):
            res = supabase.table("users").select("discord_id, win_count, participation_count").order("win_count", desc=True).execute()
            all_u = res.data; rank = next((idx+1 for idx, u in enumerate(all_u) if u['discord_id']==i2.user.id), None)
            me = next((u for u in all_u if u['discord_id']==i2.user.id), None)
            if not me: return await i2.response.send_message("등록 데이터가 없습니다.", ephemeral=True)
            e = discord.Embed(title=f"🏆 {i2.user.display_name}님의 실시간 성적", color=0xFFD700)
            e.add_field(name="랭킹", value=f"**{rank}위**", inline=True)
            e.add_field(name="승점", value=f"**{me['win_count']}점**", inline=True)
            e.add_field(name="참여 횟수", value=f"{me['participation_count']}회", inline=False)
            await i2.response.send_message(embed=e, ephemeral=True)
        btn.callback = cb; v.add_item(btn)
        await bot.get_channel(RANKING_CHANNEL_ID).send("📊 **VOLT 실시간 랭킹 보드**", view=v)
        await i.response.send_message("✅ 배포 완료", ephemeral=True)
...
class MasterDashboardView(ui.View):
    ...
    async def b_df(self, i, b):
        ...
        if unreg: return await i.response.send_message(f"미등록: {', '.join(unreg)}", ephemeral=True)
        ...
        async def cb(i2):
            ...
            await i2.channel.send("⚔️ **드래프트 시작!**", view=DraftView(p, l1, l2, [m.id for m in mbs])); await i2.response.edit_message(content="완료", view=None)
    ...
    async def b_win(self, i, b):
        ...
        async def cb(i2):
            ...
            await bot.get_channel(PUBLIC_CHANNEL_ID).send(f"🎊 **{idx}팀 승리!** 기록 완료."); await i2.response.edit_message(content="✅ 결과 기록됨", view=NextStepView())
        sel.callback = cb; v.add_item(sel); await i.response.send_message("누가 이겼나요?", view=view, ephemeral=True)
...
async def info(ctx):
    ...
    if not res.data: return await ctx.send("❗ 등록되지 않았습니다.")
...
@bot.command(name="1")
async def master(ctx): await ctx.send("⚡ **VOLT 클랜 마스터 패널**", view=MasterDashboardView())
