...
TIER_SCORE = {"아이언": 1, "브론즈": 2, "실버": 3, "골드": 4, "플래티넘": 5, "에메럴드": 6, "다이아몬드": 7, "마스터+": 9}
...
class NextActionView(ui.View):
    ...
    async def rd(self, i, b):
        ...
        async def cb(i2):
            ...
            pool = {m.id: {"n": m.display_name, "t_short": db[m.id]['tier'][0], "score": TIER_SCORE.get(db[m.id]['tier'], 3), "lines": f"{db[m.id]['main_line']}/{db[m.id]['sub_line']}"} for m in p_list if m.id not in [l.id for l in lds]}
    ...
    async def rm(self, i, b):
        ...
        await i.response.edit_message(content="🔥 리매치 기록 완료!", view=None)
...
class RankingBoardView(ui.View):
    ...
    async def show(self, i, col, title, unit):
        ...
        embed.description = f"👤 **{i.user.display_name}**님의 점수: {me[col] if me else 0}{unit}"
...
class MasterDashboardView(ui.View):
    ...
    async def b_n(self, i, b):
        v = ui.View(); sel = ui.Select(placeholder="배포 선택", options=[discord.SelectOption(label="등록", value="reg"), discord.SelectOption(label="모집", value="rec"), discord.SelectOption(label="랭킹", value="rank")])
        async def cb(i2):
            val = sel.values[0]
            if val == "rec":
    ...
    async def b_e(self, i, b): await i.response.send_message("명단 관리:", view=ParticipantEditRootView(), ephemeral=True)
