...
# 📢 선우 님 서버의 실제 채널 ID로 반드시 수정하세요!
RECRUIT_CHANNEL_ID = 1493210332598894684  # 내전 신청 & 결과 공지 채널
REGISTER_CHANNEL_ID = 1493205766209933404 # 소환사 정보 등록 채널
RANKING_CHANNEL_ID = 1493205910892318850  # 랭킹 확인 버튼 채널
...
class RegisterFlow(ui.View):
    ...
    @ui.select(placeholder="본인의 티어를 선택하세요", options=[discord.SelectOption(label=k) for k in TIER_SCORE.keys()])
    async def s_t(self, i, s):
        self.t = s.values[0]; await i.response.edit_message(content=f"✅ 선택된 티어: **{self.t}**\n이제 주라인을 선택하세요.", view=self)
    ...
    async def s_m(self, i, s):
        ...
        async def cb(i2):
            ...
            await i2.response.edit_message(content=f"🎊 **{self.rid}**님, VOLT 소환사 등록이 완료되었습니다!", view=None)
        sub.callback = cb; self.add_item(sub); await i.response.edit_message(content=f"✅ 주라인: **{self.m}**\n마지막으로 부라인을 선택하세요.", view=self)
...
class JoinView(ui.View):
    ...
    async def join(self, i, b):
        ...
        if not res.data: return await i.response.send_message("❗ 소환사 등록 채널에서 정보를 먼저 등록해주세요!", ephemeral=True)
        if i.user in active_recruitment["participants"]: active_recruitment["participants"].remove(i.user); msg = "❌ 참여 신청이 취소되었습니다."
        else:
            if len(active_recruitment["participants"]) >= active_recruitment["target"]: return await i.response.send_message("❗ 이미 정원이 가득 찼습니다.", ephemeral=True)
            active_recruitment["participants"].append(i.user); msg = "✅ 내전 참여 신청이 완료되었습니다!"
        await i.response.send_message(msg, ephemeral=True); await self.update_msg(i)

# --- [View 3: 랭킹 게시판 (2버튼 TOP 10)] ---
class RankingBoardView(ui.View):
    ...
    async def show_rank(self, i, col, title, unit):
        ...
        embed.description = f"👤 **{i.user.display_name}**님의 현재 순위: **{my_rank}위** ({my_score}{unit})"
...
class ParticipantEditView(ui.View):
    def __init__(self):
        ...
        if active_recruitment["participants"]:
            ...
            sel = ui.Select(placeholder="제외할 인원을 선택하세요", min_values=1, max_values=len(opts), options=opts)
            async def cb(i):
                ...
                if active_recruitment["message"]:
                    names = "\n".join([f"· {m.display_name}" for m in active_recruitment["participants"]])
                    await active_recruitment["message"].edit(content=f"🎮 **VOLT {active_recruitment['target']}인 내전 모집 중**\n\n**신청자 ({len(active_recruitment['participants'])}/{active_recruitment['target']}):**\n{names}")
                await i.response.send_message(f"✅ {len(to_rem)}명을 명단에서 제외했습니다.", ephemeral=True)
...
class DraftView(ui.View):
    ...
    async def pick_callback(self, i):
        if i.user.id != self.l[self.order[self.idx]].id: return await i.response.send_message("본인 픽 순서가 아닙니다.", ephemeral=True)
        ...
        else: self.create_buttons(); await i.response.edit_message(content=f"🔵 **다음 선택:** {self.l[self.order[self.idx]].mention}님", view=self)
    ...
    async def finish(self, i):
        ...
        embed = discord.Embed(title="⚔️ VOLT 내전 라인업 완료", description=f"💡 분석: {'🟦 1팀' if s1>=s2 else '🟥 2팀'} 우세 ({prob if s1>=s2 else 100-prob}%)", color=0x5865F2)
        ...
        await i.response.edit_message(content="✅ 드래프트 결과가 공지되었습니다.", view=None)

# --- [View 6: 후속 절차 (리매치 등)] ---
class NextStepView(ui.View):
    def __init__(self): super().__init__(timeout=None)
    @ui.button(label="♻️ 멤버 그대로 재드래프트", style=discord.ButtonStyle.primary)
    async def r(self, i, b):
        ...
        db = {r['discord_id']: r for r in res.data}; v = ui.View(); sel = ui.Select(placeholder="주장 2명 선택", min_values=2, max_values=2, options=[discord.SelectOption(label=u['discord_name'], value=str(u['discord_id'])) for u in db.values()])
        async def cb(i2):
            l_ids = [int(v) for v in sel.values]; leaders = [i2.guild.get_member(lid) for lid in l_ids]
            pool = {m_id: {"n": db[m_id]['discord_name'], "t_short": db[m_id]['tier'][0], "lines": f"{db[m_id]['main_line']}/{db[m_id]['sub_line']}", "score": TIER_SCORE.get(db[m_id]['tier'], 3)} for m_id in current_match["ids"] if m_id not in l_ids}
            await i2.channel.send("⚔️ **재드래프트 시작!**", view=DraftView(pool, leaders[0], leaders[1], current_match["ids"])); await i2.response.edit_message(content="시작됨", view=None)
        sel.callback = cb; v.add_item(sel); await i.followup.send("새 주장을 골라주세요:", view=v, ephemeral=True)
    @ui.button(label="🔥 팀 그대로 리매치", style=discord.ButtonStyle.success)
    async def rm(self, i, b):
        for pid in current_match["ids"]: supabase.rpc('increment_participation', {'user_id': pid}).execute()
        await bot.get_channel(RECRUIT_CHANNEL_ID).send("🔥 **팀 변경 없이 리매치 진행합니다!**"); await i.response.edit_message(content="기록 완료", view=None)

# --- [View 7: 마스터 통제실 패널] ---
class MasterDashboardView(ui.View):
    def __init__(self): super().__init__(timeout=None)

    @ui.button(label="📢 채널별 공지 배포", style=discord.ButtonStyle.primary, row=0)
    async def b_n(self, i, b):
        if not await is_admin(i): return
        v = ui.View(); sel = ui.Select(placeholder="배포할 항목 선택", options=[
        ...
        async def cb(i2):
            ...
            if val == "reg":
                ...
                async def rcb(i3):
                    ...
                    async def os(i4): await i4.response.send_message("티어와 라인을 선택하세요.", view=RegisterFlow(rid.value), ephemeral=True)
                ...
                jv = JoinView()
                msg = await bot.get_channel(RECRUIT_CHANNEL_ID).send("🎮 **내전 참여자 모집 중!**", view=jv)
                active_recruitment["message"] = msg
            elif val == "rank":
                await bot.get_channel(RANKING_CHANNEL_ID).send("📊 **VOLT 실시간 랭킹 시스템**", view=RankingBoardView())
            await i2.response.edit_message(content="✅ 해당 채널에 배포 완료!", view=None)
        sel.callback = cb; v.add_item(sel); await i.response.send_message("어디에 무엇을 배포할까요?", view=v, ephemeral=True)

    @ui.button(label="📝 신청 명단 수정", style=discord.ButtonStyle.secondary, row=0)
    async def b_edit(self, i, b):
        ...
        await i.response.send_message("제외할 유저를 선택하세요:", view=ParticipantEditView(), ephemeral=True)
    ...
    async def b_df(self, i, b):
        ...
        if len(p_list) < 10: return await i.response.send_message(f"인원이 부족합니다. (현재 {len(p_list)}/10)", ephemeral=True)
        v = ui.View(); opts = [discord.SelectOption(label=m.display_name, value=str(m.id)) for m in p_list]
        sel = ui.Select(placeholder="주장 2명을 선택하세요", min_values=2, max_values=2, options=opts)
        async def cb(i2):
            l_ids = [int(v) for v in sel.values]; leaders = [i2.guild.get_member(lid) for lid in l_ids]
            ...
            await i2.response.edit_message(content="드래프트가 시작되었습니다.", view=None)
    ...
    async def b_win(self, i, b):
        ...
        async def cb(i2):
            ...
            for pid in win_ids: 
                ...
                if u['current_streak']>=3: await bot.get_channel(RECRUIT_CHANNEL_ID).send(f"🔥 **{u['discord_name']}**님 현재 **{u['current_streak']}연승** 중입니다!")
            for pid in lose_ids: supabase.table("users").update({"current_streak": 0}).eq("discord_id", pid).execute()
            await i2.response.edit_message(content="✅ 승리 기록 완료! 다음 단계:", view=NextStepView())
    ...
    async def b_adm(self, i, b):
        ...
        async def cb(i2):
            ...
            await i2.followup.send("✅ 운영진 명단 동기화 완료!", ephemeral=True)
        sel.callback = cb; v.add_item(sel); await i.followup.send("운영진을 관리하세요:", view=v, ephemeral=True)
...
@bot.command(name="1")
async def master(ctx): await ctx.send("⚡ **VOLT 클랜 통합 마스터 패널**", view=MasterDashboardView())
