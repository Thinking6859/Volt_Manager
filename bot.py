...
class RegisterFlow(ui.View):
    ...
    @ui.select(placeholder="주라인 선택", options=[discord.SelectOption(label=l, value=l) for l in ["TOP", "JUG", "MID", "ADC", "SUP"]])
    async def s_m(self, i, s):
        self.m = s.values[0]; sub_opts = [discord.SelectOption(label=l, value=l) for l in ["TOP", "JUG", "MID", "ADC", "SUP"] if l != self.m]
...
class DraftView(ui.View):
    def __init__(self, p, l1, l2, ids):
        ...
        for idx, leader in enumerate([l1, l2], 1):
            ...
            self.teams[idx].append(f"👑 **[{d['tier'][0]}] {leader.display_name}**")
        self.create_buttons()
    ...
    async def pick_callback(self, i):
        ...
        self.teams[t_num].append(f"· **[{p['t_short']}] {p['n']}**")
        ...
        else: self.create_buttons(); await i.response.edit_message(content=f"🔵 **{self.l[self.order[self.idx]].display_name}** 주장님 선택하세요!", view=self)
    ...
    async def finish(self, i):
        ...
        await i.response.edit_message(content="✅ 드래프트 완료!", view=None)

# --- [Views: 명단 관리 및 다음 액션 (기존 기능)] ---
class ParticipantEditRootView(ui.View):
    ...
    async def add(self, i, b):
        ...
        if not db_users: return await i.response.send_message("추가 가능 유저 없음", ephemeral=True)
        ...
        sel.callback = cb; v.add_item(sel); await i.response.edit_message(content="추가할 유저 선택:", view=v)
    @ui.button(label="➖ 명단 제외", style=discord.ButtonStyle.danger)
    async def rem(self, i, b):
        if not active_recruitment["participants"]: return await i.response.send_message("인원 없음", ephemeral=True)
        ...
        sel.callback = cb; v.add_item(sel); await i.response.edit_message(content="제외할 유저 선택:", view=v)
...
class NextActionView(ui.View):
    ...
    async def rd(self, i, b):
        ...
        sel = ui.Select(placeholder="새 주장 2명 선택", min_values=2, max_values=2, options=opts)
        async def cb(i2):
            lds = [m for m in p_list if m.id in [int(v) for v in sel.values]]
            rems = [m for m in p_list if m.id not in [m.id for m in lds]]
            db = {r['discord_id']: r for r in supabase.table("users").select("*").in_("discord_id", [m.id for m in p_list]).execute().data}
            pool = {m.id: {"n": m.display_name, "t_short": db[m.id]['tier'][0], "score": TIER_SCORE.get(db[m.id]['tier'], 3), "lines": f"{db[m.id]['main_line']}/{db[m.id]['sub_line']}"} for m in rems}
            await i2.channel.send("⚔️ 재드래프트!", view=DraftView(pool, lds[0], lds[1], [m.id for m in p_list]))
    ...
    @ui.button(label="📝 명단수정 후 재시작", style=discord.ButtonStyle.secondary)
    async def ed(self, i, b): await i.response.edit_message(content="명단 수정 모드:", view=ParticipantEditRootView(follow_up=True))
    @ui.button(label="🔥 리매치", style=discord.ButtonStyle.success)
    async def rm(self, i, b):
        for pid in current_match["ids"]:
            try: supabase.rpc('increment_participation', {'user_id': pid}).execute()
            except: pass
        await i.response.edit_message(content="🔥 리매치 시작!", view=None)
...
class MasterDashboardView(ui.View):
    ...
    async def b_a(self, i, b):
        ...
        sel.callback = acb; v.add_item(sel); await i.followup.send("설정:", view=v, ephemeral=True)
...
class JoinView(ui.View):
    ...
    async def j(self, i, b):
        if i.user in active_recruitment["participants"]: active_recruitment["participants"].remove(i.user); msg = "❌ 취소"
            ...
            if not supabase.table("users").select("discord_id").eq("discord_id", i.user.id).execute().data: return await i.response.send_message("등록 먼저!", ephemeral=True)
            active_recruitment["participants"].append(i.user); msg = "✅ 신청"
