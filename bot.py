...
RECRUIT_CHANNEL_ID = 1493116057488199741 # 내전 모집/참여신청
REGISTER_CHANNEL_ID = 1493394604131745912 # 소환사 등록 센터
RANKING_CHANNEL_ID = 1493138106868568075 # 랭킹 게시판
DRAFT_CHANNEL_ID = 1493394750479270019   # ⚔️ 드래프트 진행 및 결과 전용
=======
...
>>>>>>> 89baa0b1ac0218c9d9daab578fabd9385db80b7f
...
<<<<<<< HEAD
            row_idx = 0 if i < 5 else 1 # 한 줄에 5명씩 배치
=======
...
<<<<<<< HEAD
        await i.response.edit_message(content="✅ 완료! 드래프트 채널 확인.", view=None)
...
class ParticipantEditRootView(ui.View):
    ...
    async def add(self, i, b):
        ...
        async def cb(i2):
            ...
            await i2.response.edit_message(content=f"✅ {name} 추가됨", view=NextActionView() if self.follow_up else None)
...
class NextActionView(ui.View):
    ...
    async def rm(self, i, b):
        embed = discord.Embed(title="⚔️ 동일 멤버 리매치!", color=0xed4245)
        embed.add_field(name="🟦 1팀", value="\n".join(current_match["names1"])); embed.add_field(name="🟥 2팀", value="\n".join(current_match["names2"]))
        await bot.get_channel(DRAFT_CHANNEL_ID).send(embed=embed); await i.response.edit_message(content="✅ 리매치 공지 완료", view=None)
    ...
    async def sf(self, i, b):
        ...
        await bot.get_channel(DRAFT_CHANNEL_ID).send(embed=embed); await i.response.edit_message(content="✅ 셔플 완료", view=None)
...
class MasterDashboardView(ui.View):
    ...
    async def b_n(self, i, b):
        ...
        async def cb(i2):
            val = sel.values[0]
            if val == "reg":
                reg_v = ui.View(timeout=None); btn = ui.Button(label="소환사 등록/수정", style=discord.ButtonStyle.primary)
        ...
        sel.callback = cb; v.add_item(sel); await i.response.send_message("항목 선택:", view=v, ephemeral=True)
