...
class DraftView(View):
    ...
    def make_embed(self):
        ...
        embed.set_footer(text=f"현재 순서: {self.captains[self.turn].display_name}")
        return embed
    ...
    async def pick_callback(self, interaction: discord.Interaction):
        ...
        if (len(self.teams[0]) + len(self.teams[1])) >= 10:
            ...
            await interaction.response.edit_message(content=f"✅ **드래프트 종료!**\n{mentions}", embed=self.make_embed(), view=None)
        else:
            self.update_buttons()
...
class VoltMatchView(View):
    ...
    async def select_tier(self, interaction: discord.Interaction, select: Select):
        ...
        await interaction.followup.send("티어 확인! **주 라인**을 선택하세요.", ephemeral=True)
...
async def 도움말(ctx):
    ...
    embed.add_field(name="👤 일반", value="`!명단`, `!랭킹`, `!내정보`, `!도움말`", inline=False)
    embed.add_field(name="🛠️ 운영진", value="`!내전생성`, `!드래프트 @주장1 @주장2`, `!결과기록 [1/2]`, `!강제취소 [이름]`", inline=False)
    await ctx.send(embed=embed)

@bot.command()
@commands.has_permissions(administrator=True)
async def 드래프트(ctx, cap1: discord.Member, cap2: discord.Member):
    if len(bot.waiting_list) < 10: return await ctx.send("신청자가 부족합니다.")
...
async def 결과기록(ctx, winner: int):
    if not bot.last_teams: return await ctx.send("기록할 팀 정보가 없습니다.")
    conn = get_db_conn(); cur = conn.cursor()
    ...
    await ctx.send(f"🏆 {winner}팀 승리 기록 완료!")
...
async def 명단(ctx):
    ...
    await ctx.send(f"📋 **현재 신청 명단**\n{msg}")

@bot.command()
@commands.has_permissions(administrator=True)
async def 강제취소(ctx, name: str):
    found = next((u for u, p in bot.waiting_list.items() if p['name'] == name), None)
    if found: del bot.waiting_list[found]; await ctx.send(f"🧹 {name} 제외 완료.")
    else: await ctx.send("찾을 수 없습니다.")
